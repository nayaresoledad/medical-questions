...
from .indexing_pipeline_utils import create_docs_to_embedd
...
def generate_index_pipeline(config: RetrievalExpsConfig, logger: logging.Logger) -> float:
    ...
    logger.info(f"Se han cargado datos sobre {len(medical_data):,} preguntas médicas.")
    ...
    movies_as_docs: list[Document] = create_docs_to_embedd(medical_data, config)
    ...
    movie_ids = [doc.metadata["id"] for doc in movies_as_docs]
